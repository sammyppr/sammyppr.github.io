...
5.  GroupBをダブルクリックでContentsGroupに。LayoutタブのContainer LayoutをColumn, Make this element fixed-width のチェックを外す。StyleをRemoveStyleしてからBackgroundをFlatColor,BBBBBBに。中に要素を入れづらいので、Min.Hiehgtを1000pxに。
6.  ~~GroupBの中にGroupをドラッグ(GroupC)。GroupCをダブルクリックしてGroup TweetActionに。LayoutタブのContainer LayoutをColumn, Make this element fixed-width のチェックを外す。StyleをRemoveStyleしてからBackgroundをFlatColor,FFFFFFに。LayoutタブでMarginを全て20に~~
...
6.  上のAppDatamをクリック。AllTweetsが青くなっているのを確認して、NewEntry
...
2.  Designタブをクリック
3.  Contents Groupの中~~、Group Tweetの下に~~Repeagint Groupをドラッグ&ドロップ
...
13. LayoutタブでMake this element fixed-width のチェックを外す。Min Heightを0にしましょう。RepeingGroup TweetもMin Heightを0にしておきましょう。
