...
荷物が届くと「届いたよ」と返事をすることになっていますが、届いてないと返事ができません。
...
何回送っても返事がなければ、送るのやめます。

## 動画で見てみよう
- [パケット交換方式](https://youtube.com/watch?v=GcZmdnvcCn4&t=130)
- [「TCP」「再送制御」](https://www.youtube.com/watch?v=CJP3j57V_2o)
<!--
...
それに対して、google等のページのデータを送り出すコンピュータのことを
...
一番詳しいのは
- [OSI参照モデルとは](https://www.itmanage.co.jp/column/osi-reference-model/)

と呼ばれ、7つのレイヤーに分けて考えます。

が、実際にはOSI参照モデルは概念として定着したのみで、次のTCP/IPモデルが使われています。
- [参考：OSI参照モデルの有害性](https://www.youtube.com/watch?v=9TAUTikQX7s)
...
**Webクライアント**: Google Chrome, Firefox, Safari, Opera, Edge

が代表的なソフトウェアになっています。

最近ではAI対応ブラウザがたくさん出ています。
...
-->
- [What is Request For Comments (RFC) - Network Encyclopedia](https://www.youtube.com/watch?v=TtKKKUy4WU0)
- [ずんだもん解説】RFCについて](https://www.youtube.com/watch?v=Tp1Il3igtY0)
- [The official home of RFCs](https://www.rfc-editor.org/)

### 脱線... 他にも

コンピュータの人たちは、そういうちょっと、しゃれっぽいのが大好きです。
...
データの到達確認やエラー訂正を行わず、相手にデータを「送りっぱなしにする」ことで、高速でリアルタイム性の高い通信を実現しています。
...
[参考：2024年のインターネット運用動向](https://www.nic.ad.jp/ja/materials/iw/2024/proceedings/d3/d3-1-yoshida.pdf)
...
- [HTTP/3の普及率は3割で頭打ち、実は主要ソフトが非対応](https://xtech.nikkei.com/atcl/nxt/column/18/03312/082600001/)

### QUIC
TCPの利点をこれまで説明してきましたが、HTTP/3ではQUICと呼ばれるUDPベースの技術を利用しています。
> UDPプロトコルの持つ速さと 発展性 に、TCPプロトコルの持つ信頼性を組み合わせた新しいプロトコルを利用しています。
...
現在、「h2」がHTTP/2, 「chrome-extension」がHTTP/3のようです。
