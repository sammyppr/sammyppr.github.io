...
ここで、ハードウェアを説明する前にコンピュータでどのように情報を扱っているかについて説明します。
コンピュータの中ではほとんどの場合、
- ON
- OFF
...
Macで現在利用しているM1,M2,M3,M4,M5等も64bitですが、多少面倒くさいので後で説明します。
...
[Mythbusters Demo GPU versus CPU](https://www.youtube.com/watch?v=Ge-g3xZ5bb8)
...
NPUはみなさんが持っている端末向けで、TPUはデータセンター・クラウドなどで利用されています。
...
文字通り、読み取り専用の記憶装置なので、通常の方式では書き換えることはできません。
主に、起動に必要なプログラムを格納したりするのに使われます。電源がなくなっても消えません。

### HDD(ハードディスクドライブ)
大規模な記憶装置です。
今では 4T の HDD でも 15,000 円位から購入できます。

現在、品薄により昨年(10,000円位)より値段が上がってます。
...
また、HDD に比べれば、同じ値段では SSD は容量が少ないです。 1T で 14,000 円位から購入できます。

これも、昨年に比べると倍くらいの値段になっています。

### データ容量の単位
4TのTって何? これ、4Tbyte の略です。
T はテラの略です。
...
昨年度、メディア文化論で物理的なメディアについて授業をやったのですが、本当に物理的なメディアを使う機会が減ったと思います。
...
データ転送の様子を、バスの様に、いろいろな人を拾って・運んで・降ろす、というところからきてるという説があります。
...
iPhoneやMacに搭載されているA18, M4,M5等やAndroidに搭載されているSnapdragon等は**ARMプロセッサ**と呼ばれ、少し仕組みが異なります。
...
現在は、M5/M5 Pro/M5 MAXも発表されていて、特にPro,MAXではM4よりも新しい仕組みが利用されています。
- [Apple、最も負荷の高いプロのワークフローに驚異的なパワーをもたらす、M5 ProとM5 Maxを発表](https://www.apple.com/jp/newsroom/2026/03/apple-debuts-m5-pro-and-m5-max-to-supercharge-the-most-demanding-pro-workflows/)
