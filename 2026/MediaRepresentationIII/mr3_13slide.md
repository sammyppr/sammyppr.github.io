...
---
### マスク(Premiere)
楕円形マスクツール・長方形マスクツール・ペンマスクツールを利用して設定します。

- [使えなきゃヤバい！マスクの基礎と使い方3選！！](https://www.youtube.com/watch?v=CrSGgXTMHgM) 1:10-16:30

Premiere2026では
- エフェクトコントロールで対象を選択してマスク作成
- マスクを作成後に割り当てを行う
 
のどちらかの方法に変更されました。

- [Premiere2026でマスクどこ行った？対処法（ワイプ・テロップ・ブラー）と新機能まとめ(3:09)](https://www.youtube.com/watch?v=ONJCnpmmpAk)

---
### マスク(AfterEffects)
レイヤーをダブルクリックしてから、長方形ツールなどでマスクを作成します。
エフェクトに対してマスクをかける、という考え方はない模様です。
やりたい場合には、調整レイヤーにエフェクトをかけて、そこにマスクを作成しましょう。


- [(参考)マスクについて -作成・編集・用途-【AfterEffects/チュートリアル】](https://www.youtube.com/watch?v=ku0V2PU0t1s)

もちろん、レイヤーを使ったトラックマットも利用できます。

---
### ベジェ曲線のコツ
ペンツール・パスの使い方慣れていない人は、
...
これ、大変な作業となります。フレーム数が多い場合には~~ロトスコープ~~を試しましょう。(後述)

---
### マスクトランジション
マスクの動きをうまく使うことでこんなこともできます。

[プレミアプロでできるマスクトランジションの編集方法(5:07)](https://www.youtube.com/watch?v=1quIiiezG_0)
...
---
### リアルタイム合成
編集であれば、後述のオブジェクトマスク・オブジェクトマットを利用すれば良いかもしれませんが、現在でもリアルタイムで合成したい場合にはキーイングは利用されています。

- [参考：ATEM スイッチャーを使ったクロマキー合成とマクロの活用方法(-1:02)](https://www.youtube.com/watch?v=jZBreUM34n0)
...
~~AfterEffectsのロトブラシを利用すれば、グリーンバックを利用しなくても、合成することができます。~~

---
### デジタルロトスコープでのビデオエレメントの融合(廃止)
- ~~背景からオブジェクトを分離~~
- ~~不要なオブジェクトを置き換える~~
...
---
### Object Mask/ Object Matt
- Premiere：オブジェクトマスク
- AfterEffects：オブジェクトマット

に動画の動いているものを切り抜く機能が刷新されました。
簡単に抜きたい場合にはオブジェクトマスク(Premiere)を、細かく調整したい場合にはオブジェクトマット(AfterEffects)を利用しましょう。
