...
があります。クイック書き出しは非常に便利なので積極的に使っていきましょう。
...
4. 新規プロジェクト(場所は今日作業用のフォルダ、プロジェクト名は適当に)...スキップ
5. 左下のプロジェクトパネルに2つの動画を登録(ファイル-読み込みでもOK)
6. 「1920-1080-30fps.mp4」をタイムラインにドラッグ&ドロップ
7. シーケンス設定を確認
8. シーケンスパネルを一度閉じる
9. 同様に「1280-720-24fps.mp4」をして設定を確認して見る。
...
MediaEncoderを使うことをかつてAdobeは望んでいましたが...Premiereに関してPremiere内で完結に方針転換してますね。AfterEffectsも単体で書き出しできちゃいます。

MediaEncoderは明示的にはプロキシ作成、の時に使うと思われます。(14回目)
...
7. 1フレームずつ送ると(左右矢印キー)不思議なことに気づくでしょう。
...
2. ヘッダーバーのクイック書き出しから場所・ファイル名を指定、高品質1080pHD,として H.264で書き出しましょう。(ちょこっと確認したい時)
3. 最終版を書き出す時は、書き出しタブから色々設定して書き出しましょう。
