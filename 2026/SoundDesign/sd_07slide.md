...
等、自動のものが良いと思います。(正攻法でするのは、細かい設定で本当に難しい)
音楽のマスタリングでどう変わるかを簡単にデモだけしておきます。
...
- [日本民間放送連盟 技術基準](https://www.j-ba.or.jp/stance/tech.html)
...
### 注意！
Macの人、録音するアプリ(Premiere)の音声入力がOSで利用できる状況になっているか確認しましょう。
...
- Backで戻って
- ソース - マイク...ダイナミックマイクでは+48Vはいらないですね<!--コンデンサマイクだから必要ですね-->
...
- デフォルト入力・出力で「Zoom F3 Driver」サンプルレート「48000」
...
---
### やってみよう
1. 実験として、x1...x1024まで徐々に変えていって、音が歪む様子を録音してみて確認しましょう。

2. 同一ナレーション「本日の天気は晴れ。午後から気温が上がる見込みです。」
   →代表者がmanabaでWAVで書き出して提出


# 小レポート
「マスタリングの重要性」についてまとめてmanabaにて提出

グループの代表は、「本日の天気は晴れ。午後から気温が上がる見込みです」をwavで書き出してmanabaで提出しましょう。
...
などが含まれていると、かなり学習効果が高いです。

例えば、以下のように「目的別」にセリフを用意すると、後でEQ・コンプ・ノイズ処理・音量調整・定位などを学びやすいです。
...
* 「初心者向け30分授業セット」
* 「DAW別（Audition/Reaper/Logic）練習課題」
* 「EQ練習に最適な日本語50文」
* 「ノイズ除去を学ぶための録音課題」
* 「YouTube動画風ナレーション課題」
